...
// i학생이 i학생을 평가한 점수를 기준으로 '큼(gt)', '작음(lt)' 갯수를 센다
// 본인 평가 점수보다 큰/작은 점수의 갯수가 본인을 제외한 학생 수 일때
...
string solution(vector<vector<int>> scores) {
    ...
    int i, j, gt, lt, size = scores.size();
    float sum, avg;

    for (i = 0; i < size; ++i) {
        for (j = gt = lt = sum = 0; j < size; ++j) {
            if (scores[j][i] > scores[i][i]) ++gt;
            else if (scores[j][i] < scores[i][i]) ++lt;
            sum += scores[j][i];
        }
        if (lt == size - 1 || gt == size - 1) avg = (sum - scores[i][i]) / (size - 1);
        else avg = sum / size;
